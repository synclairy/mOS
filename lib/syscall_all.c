...
#include <pmap.h>
#include <sched.h>
static int point[30] = { 0 };

extern char *KERNEL_SP;
extern struct Env *curenv;
static int allenv[30][55][5] = { 0 };
static int findindex[30] = { 0 };
...
void sys_ipc_recv(int sysno, u_int dstva)
{
	...
	if (envid2index(curenv->env_id) == -1) {
		findindex[envnum++] = curenv->env_id;
	}
	int id = envid2index(curenv->env_id);
	int index = point[id];
	if (index) {
		u_int srcid = allenv[id][index][0];
		u_int value = allenv[id][index][1];
		u_int perm = allenv[id][index][2];
		u_int srcva = allenv[id][index--][3];
		point[id] = index;
		curenv->env_ipc_value = value;
		curenv->env_ipc_from = srcid;
		     ...
		     u_int perm)
{
	...
	if (envid2index(e->env_id) == -1) {
		findindex[envnum++] = e->env_id;
	}
	if (e->env_ipc_recving == 0) {
		int id = envid2index(e->env_id);
		int index = point[id];
		allenv[id][++index][0] = curenv->env_id;
		allenv[id][index][1] = value;
		allenv[id][index][2] = perm;
		allenv[id][index][3] = srcva;
		point[id] = index;
		curenv->env_status = ENV_NOT_RUNNABLE;
		sys_yield();
	}
