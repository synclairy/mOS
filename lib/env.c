...
static struct Env_list waiting_list[3];
extern Pde *boot_pgdir;
extern char *KERNEL_SP;

static u_int asid_bitmap[2] = { 0 };	// 64

static u_int system_id = 4;
static u_int signals[3];
...
int P(struct Env *e, int s)
{
	if (e->waiting) {
		...
		LIST_INSERT_TAIL(&waiting_list[s], e, env_link);
		e->waiting = 1;
	} else {
		signals[s] -= 1;
		e->waiting = 0;
...
int V(struct Env *e, int s)
{
	if (e->waiting) {
		...
		signals[s] += 1;
		if (!LIST_EMPTY(&waiting_list[s])) {
			struct Env *new = LIST_FIRST(&waiting_list[s]);
			new->waiting = 0;
			for (i = 0; i < 100; i++) {
				if (new->using_s[i] == 0) {
					new->using_s[i] = s;
...
u_int asid_check(u_int i)
{
	int index, inner;
	index = i >> 5;
	inner = i & 31;
	return (asid_bitmap[index]) & (1 << inner);
}
...
{
	int changed = 0;
	if ((e->env_asid) >> 6 != system_id) {
		int e_hid = (e->env_asid) & 0x3f;
//              printf("asid = %x, system_id = %x, e_hid = %x, check = %d\n", e->env_asid, system_id, e_hid, asid_check(e_hid));
		if (asid_check(e_hid) == 0) {
			e->env_asid = (system_id << 6) | e_hid;
			asid_set(e_hid);
		} else {
			if (asid_all_busy() == 0) {
				int id = asid_alloc();
				e->env_asid = (system_id << 6) | id;
			} else {
				system_id += 1;
				int i;
				for (i = 0; i < 64; i++) {
...
//      printf("e_system_id = %x, e_hid = %x\n", (e->env_asid >> 6), (e->env_asid & 0x3f));
	return changed;
}
