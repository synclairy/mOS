...
u_int mkthreadid(struct Thread *t)
{
	struct Env *e;
	int r;
	if ((r = envid2env(t->env_id, &e, 0)) < 0) {
		return r;
	}
	if (t->env_id != e->env_id) {
		printf("wtf is that?\n");
	}
	u_int thread_no = (t - e->env_threads);
	return ((e->env_id << LOG2THREAD_NUM_MAX) | thread_no);
}
...
int threadid2thread(u_int threadid, struct Thread **pthread)
{
	...
	struct Env *e;
	int r = 0;
//      printf("thread id = %x", threadid);
	if (threadid == 0) {
		*pthread = curthread;
		return 0;
	}

	if ((r = envid2env(threadid >> LOG2THREAD_NUM_MAX, &e, 0)) < 0) {
		return r;
	}

	t = &e->env_threads[THREADX(threadid)];
	if (t->thread_status == ENV_FREE || t->thread_id != threadid) {
		*pthread = 0;
		return -E_BAD_ENV;
	}
	*pthread = t;
...
{
	if (e->env_thread_count >= THREAD_NUM_MAX) {
		return E_THREAD_NUM_LIMIT_EXCEED;
	}
	u_int thread_no = e->env_thread_count;
	u_int i = 0;
	while (e->env_threads[thread_no].thread_status != ENV_FREE) {
		thread_no = (thread_no + 1) % THREAD_NUM_MAX;
		if (++i >= THREAD_NUM_MAX) {
			return E_THREAD_NUM_LIMIT_EXCEED;
		}
	}
	++(e->env_thread_count);
	struct Thread *t = &e->env_threads[thread_no];
	t->env_id = e->env_id;
	t->thread_id = mkthreadid(t);
	t->thread_status = ENV_RUNNABLE;
	bzero(&t->thread_tf, sizeof(struct Trapframe));
	t->thread_tf.cp0_status = 0x1000100c;
	t->thread_tf.regs[29] = USTACKTOP - 4 * BY2PG * (THREADX(t->thread_id));
//      printf("thread %x stacktop at %x\n", t->thread_id, t->thread_tf.regs[29]);
	t->thread_canceltype = THREAD_CANCEL_ENABLE;
	t->thread_canceled = 0;
	t->thread_exit_value = 0;
	t->thread_exit_ptr = (void *)&t->thread_exit_value;
	t->thread_detach = 0;
	LIST_INIT(&t->thread_joined_list);
	*new = t;
	return 0;
}
			     ...
			     u_char * bin, u_int32_t bin_size, void *user_data)
{
	...
	for (; i + BY2PG <= bin_size; i += BY2PG) {	// BE VERY CAREFUL when using MINUS with UNSIGNED Integer !!!
		/* Hint: You should alloc a page and increase the reference count of it. */
		p = page_lookup(env->env_pgdir, tempVa, NULL);
		if (p == 0) {
			r = page_alloc(&p);
			if (r != 0) {
				return r;
			}
			page_insert(env->env_pgdir, p, tempVa, PTE_R);
		}
		//page_alloc(&p);
	...
	if (bin_size > i) {
		...
		if (i + BY2PG < sgsize) {
			//bzero(page2kva(p) + bin_size - i, i + BY2PG - bin_size); 
			i = i + BY2PG;
			tempVa += BY2PG;
		} else {
			//bzero(page2kva(p) + bin_size - i, sgsize - bin_size);
			i = sgsize;
		}
	}
...
void env_create_priority(u_char * binary, int size, int priority)
{
	...
	e->env_pri = priority;
	e->env_threads[0].thread_pri = priority;
	/* Step 3: Use load_icode() to load the named elf binary,
	   and insert it into env_sched_list using LIST_INSERT_HEAD. */
	load_icode(e, binary, size);
	LIST_INSERT_HEAD(thread_sched_list, &e->env_threads[0],
			 thread_sched_link);
...
void env_run(struct Thread *t)
{
	 ...
	 */
	//printf("popping\n");
	env_pop_tf(&(t->thread_tf), GET_ENV_ASID(curenv->env_id));
	printf("popped\n");
}
