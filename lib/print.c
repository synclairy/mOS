...
extern int PrintStruct(char *, char *, int, int, char);
...
#define 	OUTPUT(arg, s, l)  \
  { if (((l) < 0) || ((l) > LP_MAX_BUF)) { \
	...
	for (;;) {
		...
		case 'T':
			s = (char *)va_arg(ap, char *);
			length = PrintStruct(buf, s, width, ladjust, padc);
			OUTPUT(arg, buf, length);
			break;
...
int PrintStruct(char *buf, char *s, int width, int ladjust, char padc)
{
	buf[0] = '{';
	char *ptr = buf + 1;
	int *size_ptr = (int *)s;
	...
	char *char_ptr = s + 4;
	char c = *char_ptr;
	int len = PrintNum(ptr, size, 10, 0, width, ladjust, padc, 0);
	ptr[len] = ',';
	ptr = ptr + len + 1;
	len = PrintChar(ptr, c, width, ladjust);
	ptr[len] = ',';
	ptr = ptr + len + 1;
	s = s + 8;
	int i;
	for (i = 0; i < size; i++) {
		...
		len = PrintNum(ptr, k, 10, negflag, width, ladjust, padc, 0);
		ptr[len] = ',';
		ptr = ptr + len + 1;
		s += 4;
	}
