...
void mips_detect_memory()
{
	...
	npage = basemem >> PGSHIFT;	// PGSHIFT for 4kb is 12            
	npage_free = (0x03000000) >> PGSHIFT;
...
static Pte *boot_pgdir_walk(Pde * pgdir, u_long va, int create)
{
	...
	if ((*pgdir_entryp & PTE_V) == 0) {
		if (create) {
			*pgdir_entryp = PADDR(alloc(BY2PG, BY2PG, 1));	// pgdir[PDX[va]] = new alloc addr
	...
	pgtable = (Pte *) KADDR(PTE_ADDR(*pgdir_entryp));	// pgtable = pgdir[PDX[va]] -> paddr -> kva
...
void boot_map_segment(Pde * pgdir, u_long va, u_long size, u_long pa, int perm)
{
	...
	for (i = 0; i < size; i += BY2PG) {
		...
		*pgtable_entry = PTE_ADDR(pa + i) | perm | PTE_V;	// va -> addr of pgtable_entry -> set va <-> pa reflect
	}
...
void page_init(void)
{
	 ...
	 * filed to 1) */
	for (now = pages; page2kva(now) < freemem; now++) {	//  page --> pa --> kernel va < freemem
		now->pp_ref = 1;
		now->isProtected = 0;
	}

	/* Step 4: Mark the other memory as free. */
	for (now = &pages[PPN(PADDR(freemem))]; page2ppn(now) < npage; now++) {	// freemem -> pa -> ppn -> pages[ppn]
		now->pp_ref = 0;
		now->isProtected = 0;
		if (page2ppn(now) < npage_free) {
...
struct Page *page_migrate(Pde * pgdir, struct Page *pp)
{
	...
	for (i = 0; i < len; i++) {
		Pte *temp;
		(pgdir_walk(pgdir, a[i] << 12, 0, &temp));
		int perm = (*temp) & 0xfff;
		page_insert(pgdir, tp, a[i] << 12, perm);
	}
...
int page_protect(struct Page *pp) {
	...
	if (pp->isProtected == 0) {
		...
		{
			if(page_in_list == pp && (pp->isProtected == 0))
			{
				pp->isProtected = 1;
...
int inverted_page_lookup(Pde * pgdir, struct Page *pp, int vpn_buffer[])
{
	...
	int p;
	int vpn_pp;
	u_long ppn_pp = page2ppn(pp);
	/*if (PPN(PADDR(pgdir)) == ppn_pp) {
	   vpn_buffer[j++] = VPN(PADDR(pgdir);
	   } */
	// cnt[PPN(PADDR(pgdir))]++;
	for (i = 0; i < PTE2PT; i++) {
		pgdir_entry = pgdir + i;
