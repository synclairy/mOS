...
//      *init_esp = USTACKTOP;  // Change this!
	...
	if ((r =
	     syscall_mem_map(0, TMPPAGE, child, USTACKTOP - BY2PG,
			     PTE_V | PTE_R)) < 0)
		goto error;
...
{
	return (((Pde *) (*vpd))[va >> PDSHIFT] & PTE_V)
...
int usr_load_elf(int fd, Elf32_Phdr * ph, int child_envid)
{
	...
	if (offset) {
		...
		r = syscall_mem_map(child_envid, va + i, 0, BUFPAGE,
				    PTE_V | PTE_R);
		if (r < 0) {
			return r;
		}
	...
	for (pdeno = 0; pdeno < PDX(UTOP); pdeno++) {
		...
		for (pteno = 0; pteno <= PTX(~0); pteno++) {
			...
			if (((*vpt)[pn] & PTE_V) && ((*vpt)[pn] & PTE_LIBRARY)) {
				...
				if ((r =
				     syscall_mem_map(0, va, child_envid, va,
						     (PTE_V | PTE_R |
						      PTE_LIBRARY))) < 0) {
