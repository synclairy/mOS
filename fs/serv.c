...
void serve_list(u_int envid, struct Fsreq_open *rq)
{
	...
	for (i = 0; i < nblock; i++) {
		...
		for (j = 0; j < FILE2BLK; j++) {
			if (f[j].f_name[0] != '\0') {
				for (k = 0; f[j].f_name[k] != '\0'; k++, len++) {
					buf[len] = f[j].f_name[k];
				}
				buf[len++] = ' ';
			}
		}
...
void serve(void)
{
	...
	for (;;) {
		...
		case FSREQ_LIST:
			serve_list(whom, (struct Fsreq_open *)REQVA);
			break;
